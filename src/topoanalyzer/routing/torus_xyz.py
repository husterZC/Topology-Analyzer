...
from topoanalyzer.topologies.torus3d import router_id


class Torus3DXYZRoutingGenerator(RoutingGenerator):
    name = "torus_xyz"

    def validate(self, graph: TopologyGraph) -> ValidationReport:
        report = ValidationReport()
        if graph.topology_type != "torus3d":
            report.add_error(
                "torus_xyz routing requires a torus3d topology",
                topology_type=graph.topology_type,
            )
        dimensions = graph.metadata.get("dimensions", {})
        for axis in ("x", "y", "z"):
            if axis not in dimensions:
                report.add_error("torus3d graph is missing dimension metadata", axis=axis)
        return report

...
class Torus3DParams:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "Torus3DParams":
        return cls(
            x=int(data["x"]),
            y=int(data["y"]),
            z=int(data["z"]),
...
class Torus3DTopologyBuilder(TopologyBuilder):
    name = "torus3d"
    link_classes = {"x", "y", "z", "x_wrap", "y_wrap", "z_wrap"}

    def validate(self, params: Torus3DParams, links: LinkParameters) -> ValidationReport:
        report = ValidationReport()
        for axis in ("x", "y", "z"):
            value = getattr(params, axis)
            if value < 3:
                report.add_error(
                    "torus3d dimensions must be at least 3 to avoid parallel wrap links",
                    dimension=axis,
                    value=value,
                )
        if params.concentration <= 0:
            report.add_error(
                "torus3d concentration must be positive",
        ...
        for idx, override in enumerate(links.overrides):
            for endpoint_name, endpoint in (("src", override.src), ("dst", override.dst)):
                if not isinstance(endpoint, tuple) or len(endpoint) != 3:
                    report.add_error(
                        "torus3d link overrides must use 3D coordinate endpoints",
                        override_index=idx,
                ...
                if not (0 <= x < params.x and 0 <= y < params.y and 0 <= z < params.z):
                    report.add_error(
                        "torus3d link override endpoint is out of bounds",
                        override_index=idx,
                        endpoint=endpoint_name,
                        value=endpoint,
                    )
    ...
    def build(self, params: Torus3DParams, links: LinkParameters) -> TopologyGraph:
        report = self.validate(params, links)
        report.raise_if_errors()

        graph = TopologyGraph(
            name=f"torus3d_{params.x}x{params.y}x{params.z}",
